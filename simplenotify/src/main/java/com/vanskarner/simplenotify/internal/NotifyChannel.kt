...
internal object NotifyChannel {
    ...
    fun applyMessagingChannel(context: Context): String {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O
            && checkChannelNotExists(context, DEFAULT_MESSAGING_CHANNEL_ID)
        ) {
            val manager = getManager(context)
            val messagingChannel = messagingChannel(context)
            manager.createNotificationChannel(messagingChannel)
        }
        return DEFAULT_MESSAGING_CHANNEL_ID
    }
    ...
    @RequiresApi(Build.VERSION_CODES.O)
    private fun messagingChannel(context: Context): NotificationChannel {
        val channel = NotificationChannel(
            DEFAULT_MESSAGING_CHANNEL_ID,
